...
var engine = {
    ohlc: (data, symbols) => {
        ...
        data.filter(singleDay => singleDay[0] > oneWeekAgo).map(singleDay => {

            var trace = {
                y: singleDay.slice(1, 5),
                x: _.times(4, _.constant(new Date(singleDay[0]))),
                type: "box",
                marker: { color: "#8AB6B6" },
                showlegend: false
            }
        ...
        var figure = { 'data': traces, layout: { title: `Weekly OHLC <br> <i>${symbols}</i>`, } };

        plotly.getImage(figure, imgOpts, function (error, imageStream) {
            if (error) return console.log(error);

            var fileStream = fs.createWriteStream(`${imgOpts.title}.png`);
            imageStream.pipe(fileStream);
        });
    },
    candleStick: (data, symbols) => {
        var imgOpts = {
            format: 'png',
            width: 1000,
            height: 500,
